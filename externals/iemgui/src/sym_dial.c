...
iemgui written by Thomas Musil, Copyright (c) IEM KUG Graz Austria 2000 - 2018 */
/* not ready */
...
static void sym_dial_draw_swap(t_sym_dial *x, t_glist *glist, int swap)
{
  ...
  sys_vgui(".x%lx.c itemconfigure %lxSYMBOL -fill #%06x\n",
    canvas, x,
...
static void sym_dial_draw_update(t_sym_dial *x, t_glist *glist)
{
  if (glist_isvisible(glist))
  {
    ...
    sys_vgui(".x%lx.c itemconfigure %lxSYMBOL -fill #%06x -text {%s} \n",
...
static void sym_dial_draw_new(t_sym_dial *x, t_glist *glist)
{
  ...
  sys_vgui(".x%lx.c create polygon %d %d %d %d %d %d %d %d %d %d %d %d -outline #%06x -fill #%06x -tags %lxBASE\n",
    canvas, xpos-1, ypos,
    ...
    sys_vgui(".x%lx.c create text %d %d -text {%s} -anchor w \
      -font {{%s} %d %s} -fill #%06x -tags %lxLABEL\n",
      canvas, xpos+x->x_gui.x_ldx, ypos+x->x_gui.x_ldy,
      strcmp(x->x_gui.x_lab->s_name, "empty")?x->x_gui.x_lab->s_name:"",
      x->x_gui.x_font, x->x_gui.x_fontsize*IEMGUI_ZOOM(x), sys_fontweight, x->x_gui.x_lcol, x);
      sys_vgui(".x%lx.c create text %d %d -text {%s} -anchor w \
        -font {%s %d bold} -fill #%06x -tags %lxSYMBOL\n",
...
static void sym_dial_draw_config(t_sym_dial* x,t_glist* glist)
{
  ...
  sys_vgui(".x%lx.c itemconfigure %lxLABEL -font {{%s} %d %s} -fill #%06x -text {%s} \n",
    ...
    strcmp(x->x_gui.x_lab->s_name, "empty")?x->x_gui.x_lab->s_name:"");
  sys_vgui(".x%lx.c itemconfigure %lxSYMBOL -font {{%s} %d %s} -fill #%06x \n",
    canvas, x, x->x_gui.x_font, x->x_gui.x_fontsize*IEMGUI_ZOOM(x), sys_fontweight,
    x->x_gui.x_fsf.x_selected?IEM_GUI_COLOR_SELECTED:x->x_gui.x_fcol);
  sys_vgui(".x%lx.c itemconfigure %lxBASE -fill #%06x\n", canvas,
    x, x->x_gui.x_bcol);
...
#if defined(PD_MAJOR_VERSION) && (PD_MINOR_VERSION < 47)
  int bflcol[3];
...
static void sym_dial_clear(t_sym_dial *x)
{
  x->x_index = 0;
  x->x_ac = 1;
...
static void *sym_dial_new(t_symbol *s, int argc, t_atom *argv)
{
  ...
  int w=6, h=14, ac=0, i, j, arg_ok=0;
  ...
  colat[0].a_type = A_FLOAT;
  colat[1].a_type = A_FLOAT;
  colat[2].a_type = A_FLOAT;
  colat[0].a_w.w_float = -262144;
  colat[1].a_w.w_float = -1;
  colat[2].a_w.w_float = -1;
  iemgui_all_loadcolors(&x->x_gui, colat+0, colat+1, colat+2);
#endif
  ...
  if((argc >= 16)&&IS_A_FLOAT(argv,0)&&IS_A_FLOAT(argv,1)
    ...
    &&(IS_A_SYMBOL(argv,11)||IS_A_FLOAT(argv,11))
    &&(IS_A_SYMBOL(argv,12)||IS_A_FLOAT(argv,12))
    &&(IS_A_SYMBOL(argv,13)||IS_A_FLOAT(argv,13))
...
void sym_dial_setup(void)
{
  ...
  class_addmethod(sym_dial_class, (t_method)sym_dial_clear, gensym("clear"), 0);
