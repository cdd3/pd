...
static void hfadl_scale_color(t_hfadl_scale *x, t_symbol *s, int argc, t_atom *argv)
{
  ...
  {
    int col = (int)atom_getintarg(0, argc, argv);
    ...
    while(col < 0)
      ...
      x->x_gui.x_lcol = my_iemgui_color_hex[col];
    }
    else
      x->x_gui.x_lcol = (-1 - col) & 0xffffff;
...
static void *hfadl_scale_new(t_symbol *s, int argc, t_atom *argv)
{
  ...
  if((argc >= 1)&&IS_A_FLOAT(argv,0))
  {
    ...
    {
      col = -1 - col;
      x->x_gui.x_lcol = ((col & 0x3f000) << 6)|((col & 0xfc0) << 4)|((col & 0x3f) << 2);
    }
  }
  else
    x->x_gui.x_lcol = 0;
