...
static void soundfile_info_read(t_soundfile_info *x, t_symbol *filename)
{
  ...
  {
	int i, n4, filesize, read_chars, header_size=0, bytesperframe, n_frames;
    t_atom *at;
    char *cvec;
    unsigned long ul_chunk_size, ul_sr, ul_frames;
    short ss_format, ss_ch, ss_bytesperframe;
...
soundfile_info_data:
      ul_frames = soundfile_info_string_to_uint32(cvec + 4) / bytesperframe;
      ...
      /* if the file is truncated, report the truncated size */
      if (ul_frames < (unsigned long)n_frames)
          n_frames = ul_frames;
...
void soundfile_info_setup(void)
{
  ...
  //class_addmethod(soundfile_info_class, (t_method)soundfile_info_overwrite_sr, gensym("overwrite_sr"), A_SYMBOL, A_FLOAT, 0); // no ready yet
}
