...
static void peakenv_hold_tilde_ft1(t_peakenv_hold_tilde *x, t_floatarg t_hold)/* hold-time in ms */
{
  double dhold;
  ...
  dhold = x->x_milli_sr * x->x_holdtime;
  if(dhold > 2147483647.0)
    dhold = 2147483647.0;
  x->x_n_hold = (int)(dhold + 0.5);
}

static void peakenv_hold_tilde_ft2(t_peakenv_hold_tilde *x, t_floatarg t_rel)/* release-time in ms */
{
  if(t_rel < 0.0)
    t_rel = 0.0;
  x->x_releasetime = (double)t_rel;
  x->x_c1 = exp(-1.0/(x->x_milli_sr * x->x_releasetime));
}
...
{
  x->x_milli_sr = (double)sp[0]->s_sr * 0.001;
