  ...
  if(x->event_mask)
  {
    ...
    if(x->counter_lg)
    {
      if(x->counter_lg <= 1)
      {
        x->cur_lg = x->end_lg;
        x->counter_lg = 0;
        x->event_mask &= 62;/*set event_mask_bit 0 = 0 , 111110 */
      }
      else
      {
        x->counter_lg--;
    ...
    if(x->counter_lf)
    {
      if(x->counter_lf <= 1)
      {
        x->cur_lf = x->end_lf;
        x->counter_lf = 0;
        x->event_mask &= 61;/*set event_mask_bit 1 = 0 , 111101 */
      }
      else
      {
        x->counter_lf--;
    ...
    if(x->counter_mg)
    {
      if(x->counter_mg <= 1)
      {
        x->cur_mg = x->end_mg;
        x->counter_mg = 0;
        x->event_mask &= 59;/*set event_mask_bit 2 = 0 , 111011 */
      }
      else
      {
        x->counter_mg--;
    ...
    if(x->counter_hf)
    {
      if(x->counter_hf <= 1)
      {
        x->cur_hf = x->end_hf;
        x->counter_hf = 0;
        x->event_mask &= 55;/*set event_mask_bit 3 = 0 , 110111 */
      }
      else
      {
        x->counter_hf--;
    ...
    if(x->counter_hg)
    {
      if(x->counter_hg <= 1)
      {
        x->cur_hg = x->end_hg;
        x->counter_hg = 0;
        x->event_mask &= 47;/*set event_mask_bit 4 = 0 , 101111 */
      }
      else
      {
        x->counter_hg--;
    ...
    {
      x->counter_sr = 0;
      x->event_mask &= 31;/*set event_mask_bit 5 = 0 , 011111 */
    }
...
static void hml_shelf_tilde_ft5(t_hml_shelf_tilde *x, t_floatarg hl)
{
  ...
  if(hg != x->cur_hg)
  {
    ...
    x->event_mask |= 16;/*set event_mask_bit 4 = 1 , 010000 */
  }
}
...
static void hml_shelf_tilde_ft4(t_hml_shelf_tilde *x, t_floatarg hf)
{
  ...
  if(sqhf != x->cur_hf)
  {
    ...
    x->event_mask |= 8;/*set event_mask_bit 3 = 1 , 001000 */
  }
}
...
static void hml_shelf_tilde_ft3(t_hml_shelf_tilde *x, t_floatarg ml)
{
  ...
  if(mg != x->cur_mg)
  {
    ...
    x->event_mask |= 4;/*set event_mask_bit 2 = 1 , 000100 */
  }
}
...
static void hml_shelf_tilde_ft2(t_hml_shelf_tilde *x, t_floatarg lf)
{
  ...
  if(sqlf != x->cur_lf)
  {
    ...
    x->event_mask |= 2;/*set event_mask_bit 1 = 1 , 000010 */
  }
}
...
static void hml_shelf_tilde_ft1(t_hml_shelf_tilde *x, t_floatarg ll)
{
  ...
  if(lg != x->cur_lg)
  {
    ...
    x->event_mask |= 1;/*set event_mask_bit 0 = 1 , 000001 */
  }
}
...
static void hml_shelf_tilde_dsp(t_hml_shelf_tilde *x, t_signal **sp)
{
  ...
  if(x->x_old_sr != sp[0]->s_sr)
  {
    ...
    x->counter_sr = 1;
    x->event_mask |= 32;/*set event_mask_bit 5 = 1 , 100000 */
  }
  if(n&7)
