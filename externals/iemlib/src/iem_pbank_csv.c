...
static void iem_pbank_csv_write(t_iem_pbank_csv *x, t_symbol *filename, t_symbol *format)
{
  ...
  else
  {
    ...
    {
      sfmt[0] = format->s_name[0];
      sfmt[1] = format->s_name[1];
      sfmt[2] = 0;
      for(p=0; p<IEM_PBANK_FORMAT_SIZE; p++)
      {
        if((sfmt[p] >= 'A')&&(sfmt[p] <= 'Z'))
          sfmt[p] += 'a' - 'A';
      }
      
      if((sfmt[IEM_PBANK_ITEM_SEPARATOR] == 'b')
         ||(sfmt[IEM_PBANK_ITEM_SEPARATOR] == 's')
         ||(sfmt[IEM_PBANK_ITEM_SEPARATOR] == 't'))
        mode[IEM_PBANK_ITEM_SEPARATOR] = sfmt[IEM_PBANK_ITEM_SEPARATOR];
      
      if((sfmt[IEM_PBANK_END_OF_LINE] == 'b')
         ||(sfmt[IEM_PBANK_END_OF_LINE] == 's')
         ||(sfmt[IEM_PBANK_END_OF_LINE] == 't')
         ||(sfmt[IEM_PBANK_END_OF_LINE] == 'r'))
        mode[IEM_PBANK_END_OF_LINE] = sfmt[IEM_PBANK_END_OF_LINE];
    }
    else
