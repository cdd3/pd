...
struct _filter_tilde_sp_para
{
  ...
  int       event_mask;// a four bit mask: Bit 0 is HIGH during frequency ramp, Bit 1 is HIGH during damping ramp, Bit 2 is HIGH during frequency shift ramp, Bit 3 is HIGH if samplerate changed
  ...
  if(x->event_mask)
  {
    if(x->counter_f)
    {
      ...
      if(x->counter_f <= 1)
      {
        x->cur_f = x->end_f;
        x->counter_f = 0;
        x->event_mask &= 14;/*set event_mask_bit 0 = 0 , 1110 */
      }
      else
      {
        x->counter_f--;
    ...
    if(x->counter_a)
    {
      if(x->counter_a <= 1)
      {
        x->cur_a = x->end_a;
        x->counter_a = 0;
        x->event_mask &= 13;/*set event_mask_bit 1 = 0 , 1101*/
      }
      else
      {
        x->counter_a--;
    ...
    if(x->counter_b)
    {
      if(x->counter_b <= 1)
      {
        x->cur_b = x->end_b;
        x->counter_b = 0;
        x->event_mask &= 11;/*set event_mask_bit 2 = 0 , 1011*/
      }
      else
      {
        x->counter_b--;
    ...
    {
      x->counter_sr = 0;
      x->event_mask &= 7;/*set event_mask_bit 3 = 0 , 0111*/
    }
...
static t_int *filter_tilde_sp_perf8_2o(t_int *w)
{
  ...
  /* NAN protect */
  if(IEM_DENORMAL_T(wn[0]))
    wn[0] = 0.0f;
  if(IEM_DENORMAL_T(wn[1]))
    wn[1] = 0.0f;
...
static t_int *filter_tilde_dp_perform_2o(t_int *w)
{
  ...
  /* NAN protect */
    if(IEM_DENORMAL_D(wn2))
      wn2 = 0.0f;
    if(IEM_DENORMAL_D(wn1))
      wn1 = 0.0f;
...
static t_int *filter_tilde_dp_perf8_2o(t_int *w)
{
  ...
  /* NAN protect */
    if(IEM_DENORMAL_D(wn[0]))
      wn[0] = 0.0f;
    if(IEM_DENORMAL_D(wn[1]))
      wn[1] = 0.0f;
...
static void filter_tilde_ft3(t_filter_tilde *x, t_floatarg b)
{
  if(x->x_precision_dp1_sp0)
  {
    ...
    if(db != x->x_para.dp.cur_b)
    {
      ...
      x->x_para.dp.event_mask |= 4;/*set event_mask_bit 2 = 1 , 0100 */
    }
  }
  else
  {
    ...
    if(sb != x->x_para.sp.cur_b)
    {
      ...
      x->x_para.sp.event_mask |= 4;/*set event_mask_bit 2 = 1 , 0100 */
    }
  }
}
...
static void filter_tilde_ft2(t_filter_tilde *x, t_floatarg a)
{
  if(x->x_precision_dp1_sp0)
  {
    ...
    if(da != x->x_para.dp.cur_a)
    {
      ...
      x->x_para.dp.event_mask |= 2;/*set event_mask_bit 1 = 1 , 0010 */
    }
  }
  else
  {
    ...
    if(sa != x->x_para.sp.cur_a)
    {
      ...
      x->x_para.sp.event_mask |= 2;/*set event_mask_bit 1 = 1 , 0010 */
    }
  }
}
...
static void filter_tilde_ft1(t_filter_tilde *x, t_floatarg f)
{
  if(x->x_precision_dp1_sp0)
  {
    ...
    if(df != x->x_para.dp.cur_f)
    {
      ...
      x->x_para.dp.event_mask |= 1;/*set event_mask_bit 0 = 1 , 0001 */
    }
  }
  else
  {
    ...
    if(sf != x->x_para.sp.cur_f)
    {
      ...
      x->x_para.sp.event_mask |= 1;/*set event_mask_bit 0 = 1 , 0001 */
    }
  }
}
...
static void filter_tilde_dsp(t_filter_tilde *x, t_signal **sp)
{
  ...
  if(x->x_precision_dp1_sp0)
  {
    ...
    if(x->x_old_sr != sp[0]->s_sr)
    {
      x->x_old_sr = sp[0]->s_sr;
      x->x_para.dp.counter_sr = 1;
      x->x_para.dp.event_mask |= 8;/*set event_mask_bit 3 = 1 , 1000 */
      /*filter_tilde_dsp_dp_tick(&x->x_para.dp);*/
    }
  ...
  else
  {
    ...
    {
      x->x_old_sr = sp[0]->s_sr;
      x->x_para.sp.counter_sr = 1;
      x->x_para.sp.event_mask |= 8;/*set event_mask_bit 3 = 1 , 1000 */
      /*filter_tilde_dsp_sp_tick(&x->x_para.sp);*/
    }
